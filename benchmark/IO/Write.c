...
int main(int argc, char *argv[])
{
    ...
    size_t total_size = file_size_mb * 1024 * 1024; // Total buffer size for the specified MB

    // Open the file using the open syscall with O_DIRECT
    int fd = open("test.txt", O_WRONLY | O_CREAT | O_TRUNC | O_DIRECT, S_IRUSR | S_IWUSR);
    if (fd == -1)
    {
        return 1;
    }
    ...
    {
        size_t to_write = (total_size - written_total > WRITE_CHUNK_SIZE) ? WRITE_CHUNK_SIZE : (total_size - written_total);
