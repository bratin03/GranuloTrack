/**
 * @file kmalloc_lkm.c
 * @brief Linux Kernel Module for per-process memory allocation via /proc interface.
 *
 * This module allows processes to allocate and automatically free memory using `kmalloc`
 * by writing a size to `/proc/kmalloc_lkm`. Memory is automatically freed when the
...
static struct process_node *process_list = NULL; /**< Linked list of processes */
